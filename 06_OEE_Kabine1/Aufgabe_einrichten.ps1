...
Write-Host ""
Write-Host " EINHAUS Lackierung - OEE Tagesbericht einrichten" -ForegroundColor Cyan
Write-Host " ==================================================="
Write-Host ""
...
Get-ScheduledTask -TaskName 'EINHAUS OEE Tagesbericht' -ErrorAction SilentlyContinue |
...
try {
    ...
    $trigger = New-ScheduledTaskTrigger -Daily -At '08:40AM'
    ...
    $task = Register-ScheduledTask `
        -TaskName    'EINHAUS OEE Tagesbericht' `
        -Action      $action `
        -Trigger     $trigger `
        -Settings    $settings `
        -Description 'Sendet taeglich um 07:00 Uhr den OEE-Tagesbericht per Outlook fuer Lackierroboter Kabine 1' `
        -Force

    Write-Host " Erfolgreich eingerichtet!" -ForegroundColor Green
    Write-Host ""
    Write-Host ("  Name:      " + $task.TaskName)
    Write-Host  "  Zeitplan:  Taeglich um 08:40 Uhr"
